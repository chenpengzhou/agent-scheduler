...
from ..models.demand import Demand, DemandStage, DemandStatus, DemandPriority
...
class PipelineService:
    ...
    def transition(self, demand_id: str, target_stage: str, reason: str = "") -> Dict:
        ...
        if not self.can_transition(current_stage, target_stage):
            return {
                "success": False, 
                "error": f"Cannot transition from {current_stage} to {target_stage}"
            }
    ...
    def get_demands_by_stage(self, stage: str) -> List[Dict]:
        """获取指定阶段的需求列表"""
        return [
            d for d in self.demands_db.values()
    ...
    def bulk_transition(self, demand_ids: List[str], target_stage: str) -> Dict:
        ...
        return {
            ...
            "failed": len(demand_ids) - success_count,
            "results": results
        }
